            ...
            @if (auth()->check())
                @if (in_array(auth()->user()->role, ['admin', 'user']))
                    ...
                    class="flex items-center justify-between bg-white hover:bg-blue-50 border border-gray-200 rounded-lg shadow px-6 py-4 transition duration-200">
                        <div>
                            <div class="text-lg font-semibold text-gray-800">Workcenter</div>
                            <div class="text-sm text-gray-500">List workcenter</div>
                        </div>
                        <div class="text-sm text-blue-600 font-medium">Detail →</div>
                    </a>
                @endif
            @endif
            </div>
